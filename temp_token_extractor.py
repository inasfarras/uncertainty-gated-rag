...
with open(file_path, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        if entry.get('qid') == target_qid:
            ...
            if 'draft' in entry and entry.get('usage') and entry['usage'].get('completion_tokens') is not None:
                # Store this, as it's the latest generation step we've seen
                completion_tokens_for_final_answer = entry['usage'].get('completion_tokens')
