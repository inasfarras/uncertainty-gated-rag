...
class RetrievalAgent:
    def __init__(self, retriever: Optional[VectorRetriever] = None, faiss_dir: Optional[str] = None):
        # Default to configured FAISS index path to avoid mismatches
        self.retriever = retriever or VectorRetriever(faiss_dir or settings.FAISS_INDEX_PATH)
    ...
    def explore(
        ...
        try:
            ...
            em_ctx = embed_texts([c.get("text", "") for c in contexts]).astype(np.float32)
            sims = em_ctx @ em_q
